...
const OutpointIndexer = require('../indexer/outpointindexer');
const BlockTimestampIndexer = require('../indexer/blocktimestampindexer');
...
class FullNode extends Node {
  ...
  constructor(options) {
    ...
    if (this.config.bool('index-outpoint'))
      this.outpointindex = new OutpointIndexer({
        network: this.network,
        logger: this.logger,
        chain: this.chain,
        memory: this.config.bool('memory'),
    ...
    this.blocktimestampindex = null;
    if (this.config.bool('index-blocktimestamp'))
      this.blocktimestampindex = new BlockTimestampIndexer({
        network: this.network,
        logger: this.logger,
        chain: this.chain,
        memory: this.config.bool('memory'),
  ...
  init() {
    ...
    if (this.outpointindex)
      this.outpointindex.on('error', err => this.error(err));

    if (this.blocktimestampindex)
      this.blocktimestampindex.on('error', err => this.error(err));
  ...
  async open() {
    ...
    if (this.outpointindex)
      await this.outpointindex.open();

    if (this.blocktimestampindex)
      await this.blocktimestampindex.open();
  ...
  async close() {
    ...
    if (this.outpointindex)
      await this.outpointindex.close();

    if (this.blocktimestampindex)
      await this.blocktimestampindex.close();
  ...
  /**
   * Retrieve all blocks between given timestamp. If endTimestamp is ommitted, assumes that startTimestamp = endTimestamp
   * @param startTimestamp {number} seconds since UNIX
   * @param endTimestamp {number} optional, seconds since UNIX
   * @return {Promise} - Returns {@link Block[]}
   */
  async getBlocksByTimestamp(startTimestamp, endTimestamp) {
    const blockHashes = await this.blocktimestampindex.getBlockHashesByTimestamp(startTimestamp, endTimestamp);
